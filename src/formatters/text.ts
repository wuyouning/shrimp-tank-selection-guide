...
import { Language, PreflightReport, ScoreBreakdownItem } from '../types';
...
const EXACT_TEXT: Record<Language, Record<string, string>> = {
  en: {
...
const PREFIX_TEXT: Record<Language, Record<string, string>> = {
  en: {
...
export function formatText(report: PreflightReport, verbose = false, lang: Language = 'en'): string {
  ...
  lines.push(pc.bold(`${t.title} v${report.version}`));
  lines.push(
  ...
  if (homebrew) {
    lines.push(
      `- ${t.homebrew}: ${homebrew.detected ? homebrew.version || t.detected : t.notDetected}${!homebrew.detected && homebrew.installUrl ? ` (${homebrew.installUrl})` : ''}`,
    );
  }
  ...
  lines.push(pc.bold(t.hardware));
  lines.push(`- ${t.cpu}: ${report.hardware.cpuModel}`);
  lines.push(`- ${t.cores}: ${report.hardware.cpuCores}`);
  lines.push(`- ${t.memory}: ${formatBytes(report.hardware.totalMemoryBytes)} ${t.total} / ${formatBytes(report.hardware.freeMemoryBytes)} ${t.free}`);
  lines.push(`- ${t.disk}: ${formatBytes(report.hardware.diskFreeBytes)} ${t.free} / ${formatBytes(report.hardware.diskTotalBytes)} ${t.total}`);
  if (report.hardware.loadAverage?.length) {
    lines.push(`- ${t.loadAvg}: ${report.hardware.loadAverage.map((x) => x.toFixed(2)).join(' / ')}`);
  }
  lines.push('');

  lines.push(pc.bold(t.dependencies));
  ...
  for (const net of report.network.checks) {
    ...
    if (verbose && net.details) lines.push(pc.dim(`  ${t.details}: ${net.details}`));
  }
  lines.push('');

  lines.push(pc.bold(t.fit));
  ...
  if (verbose) {
    lines.push(pc.dim(`${t.timestamp}: ${report.timestamp}`));
    lines.push(pc.dim(`${t.profile}: ${report.profile}`));
    lines.push(pc.dim(`${t.language}: ${report.language}`));
    lines.push(pc.dim(`${t.rawPlatform}: ${report.host.platform}`));
  }
